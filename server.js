...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@392279241380724737>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  ...
  if (command === prefix + "unmute") {
    ...
    if (!muteRole)
      return message
        .reply("** You have no role muted 'Muted' **")
        .catch(console.error);
    ...
    if (message.author.id == message.guild.ownerID) {
      message.author
        .send(
          `   
**__General orders__** ✽
**\`${prefix}bot\` : about the bot** 
**\`${prefix}user\` : about you** 
**\`${prefix}avatar\` : To view your photo** 
**\`${prefix}color\` : To choose your color in the server **
**\`${prefix}colors\` : Change your color ** 
**\`${prefix}inf\` : The your invite in the server**

**__Admin orders__** ✽ 
**\`${prefix}clear\` : To clear the chat** 
**\`${prefix}ban\` : To ban someone** 
**\`${prefix}kick\` : To kick someone** 
**\`${prefix}open\` : Opening the chat** 
**\`${prefix}close\` : Chat lock** 
**\`${prefix}mute\` : To mute someone** 
**\`${prefix}unmute\` : To unmute someone** 
**\`${prefix}say\` : The bot repeats your message**
**\`${prefix}move\` : To move someone to me channel**
**\`${prefix}ccolors\` : To make random colors **
**\`${prefix}reply\` : To make an automatic message**
**\`${prefix}setLog\` :  Select channel log**
**\`${prefix}setby\` : Select channel departure**
**\`${prefix}setWelcomer <channel name>\` : Select channel a welcome  **
**\`${prefix}setMessage\` : Select a welcome message **
**\`${prefix}setVc\` <channel name> : Select channel Voice Online **
**\`${prefix}vc off\` : Close channel Voice Online **
**\`${prefix}ls\` : Show all server bots**
**\`${prefix}role\` : To give someone a role**
**\`${prefix}role all\` : To give everyone a role**
  `
        )
        .then(() => {
          message.author.send(`
...
**__Music orders__** ✽  
**\`${prefix}Play\` : plays the provided song [p] **
**\`${prefix}Pause\` : Pause the song**
**\`${prefix}Resume\` : Complete the song  **
**\`${prefix}stop\` : To stop the song and exit the bot from the voiceChannel**
**\`${prefix}forceskip\` : votes to skip the current song**
**\`${prefix}Queue\` : shows the available playlists  **
**\`${prefix}skipto\` : To skip the song to the next song in the next music queue**
**\`${prefix}Skip\` : votes to skip the current song **
**\`${prefix}Volume\` : sets or shows volume [vol]  **
**\`${prefix}np\` : shows the song that is currently playing [np] **
**\`${prefix}repeat\` :  re-adds music to the queue when finished **
**\`${prefix}Leave\` : Exit the voiceChannel**

**__Protection orders__** ✽  
**\`${prefix}settings limitsban\` : Set the limits ban the someone **
**\`${prefix}settings limitskick\` : Set the limits kick the someone **
**\`${prefix}settings limitsroleD\` : Set the limits roles to he is delete to ban the someone **
**\`${prefix}settings limitsroleC\` : Set the limits role to he is made to ban the someone **
**\`${prefix}settings limitschannelD\` : Set the limits channel to he is delete to ban the someone **
**\`${prefix}settings limitstime\` : Set the limits time the someone to ban**
**\`${prefix}antibots on\` : operation deny entry bots**
**\`${prefix}antibots off\` : operation open entry bots**
...
`);
        ...
        .catch(() => {
          return message.channel
            .send(
              "**Open the your DM, so that we can send orders to you**"
            )
            .then(() => {
